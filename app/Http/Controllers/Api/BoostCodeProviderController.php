...
class BoostCodeProviderController extends Controller
{
    ...
    public function useBoostCode(Request $request)
    {
        ...
        if (Auth::guard('api')->check()) {
            ...
            if (!$user->alreadyBoostedThisMonth()) {
                ...
                if (count($usedBoostCodes) <= 0) {
                    ...
                    $userBoostCode = $boostCodeProvider->getBoostCodeOfUser($user);
                ...
                //Already used a boost code this month
                return response()->json([
                    'status' => 'Error',
                    'message' => 'Already used a boost code this month. Cannot use any more boost codes again this month.'
                ], 401);
            }
        } else {
            //Api Authentication Failed
