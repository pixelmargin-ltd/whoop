...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function registerWithApi(Request $request)
    {
        ...
        if ($request->exists('home_button_unique_code')) {
            //If this parameter exists then the new registration is
            //From an auto-fill so accept invite and unlock home button
            $invitation = $user->getOneActiveInvitation();
            $home_button = $user->home_button;
            if ($invitation instanceof Invitation && $home_button == $invitation->home_button &&
                $home_button->unique_code === $request->input('home_button_unique_code')) {
                $invitation->acceptInvitation($user);
            } else {
                return response()->json([
                    'status' => 'Info',
                    'message' => 'User registered successfully. Invite Unique code mismatch found. Please try unlocking home button again.',
