...
class User extends Authenticatable
{
    ...
    /**
     * Gets one active invitation of current user
     *
     * @return Invitation
     */
    public function getOneActiveInvitation()
    {
        return Invitation::where('to_email', '=', $this->email)
            ->where('status', '=', Invitation::STATUS_PENDING)->first();
    }
    ...
    /**
     * Function return true if user already used a boost code this month.
     *
     * @return bool
     */
    public function alreadyBoostedThisMonth()
    {
        $from = new Carbon('first day of this month');
        $to = new Carbon('first day of next month');

        $boostCode = UserBoostCode::where('user_id', '=', $this->id)
            ->where('status', '=', UserBoostCode::STATUS_USED)
            ->whereBetween('updated_at', [$from, $to])->first();
        return $boostCode instanceof UserBoostCode;
    }
}
